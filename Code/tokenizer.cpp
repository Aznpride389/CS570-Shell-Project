...
list<string> parse(char* buffer) {
	...
	/* state0: nothing special, 
	 *state1: hit a double quote, 
	 *state2: hit a single quote, 
	 *state3: handles escape characters within single quote
	 */
	...
	while (1) {
		...
		if (state == 0) {
			if (c == '\0') {
				...
				cout << "Unrecognized: " << c << endl; 
				//in the case of characters not caught in the "buckets" below
